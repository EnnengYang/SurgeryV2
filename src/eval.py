...
from modeling import ImageClassifier, ImageClassifierWithMapping, ImageClassifierWithMapping_V2
...
def eval_single_dataset_preprocess_mapping_head_V2(image_encoder, head, dataset_name, args, down_projs, up_projs):
    # model = ImageClassifier(image_encoder, head)
    model = ImageClassifierWithMapping_V2(image_encoder, head, down_projs, up_projs)
    model.eval()
    dataset = get_dataset(dataset_name, model.val_preprocess, location=args.data_location,  batch_size=args.batch_size)
    dataloader = get_dataloader(dataset, is_train=False, args=args, image_encoder=None)
    device = args.device
